	subroutine v0_bb2_c1a1c1a1(L, n_k, dn_dt, V_q, V_q_dependent, rta)
	integer L
	   	...
	   	do m2x = -L2, L2-1
			...
			do m3x = -L2, L2-1
				...
				if (e1+e2 .EQ. e3+e4) then
					...
					if (rta) then
						term = 1*(n2+1)*n3*n4 - 1*n2*(n3+1)*(n4+1)
						if ( (m1x.EQ.m4x) .AND. (m1y.EQ.m4y) .AND. (m1z.EQ.m4z) ) then
							term = term + (n1+1)*(n2+1)*n3*1 - n1*n2*(n3+1)*1
						end if
					else
						term = (n1+1)*(n2+1)*n3*n4 - n1*n2*(n3+1)*(n4+1)
					end if
	...
	end subroutine v0_bb2_c1a1c1a1
